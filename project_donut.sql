...
SELECT AVG(years_old), 
       MAX(donuts_eaten), 
       MIN(donuts_eaten) 
    FROM winstons_donut_logs;
    
SELECT status as Category 
FROM winstons_donut_logs WHERE reason = '';

SELECT status as Category, reason FROM winstons_donut_logs GROUP BY years_old HAVING reason = 'Braces';

SELECT status, reason, donuts_eaten FROM winstons_donut_logs 
WHERE years_old > 24 AND donuts_eaten < 900;

SELECT status, reason, donuts_eaten FROM winstons_donut_logs 
WHERE years_old < 24 OR donuts_eaten < 900;
